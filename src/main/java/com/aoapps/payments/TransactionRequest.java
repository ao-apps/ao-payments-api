...
public class TransactionRequest implements Cloneable {
	...
	public TransactionRequest() {
		setCurrency(Currency.getInstance(PACKAGE_RESOURCES.getMessage("TransactionRequest.currency.default")));
	}
	...
	public TransactionRequest(
		...
		if(currency==null) setCurrency(Currency.getInstance(PACKAGE_RESOURCES.getMessage("TransactionRequest.currency.default")));
	...
	public void setAmount(BigDecimal amount) throws IllegalArgumentException {
		if(amount==null) {
			...
			if(amount.compareTo(BigDecimal.ZERO)<=0) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "TransactionRequest.setAmount.amount.lessThanEqualZero");
			try {
				this.amount = amount.setScale(currency.getDefaultFractionDigits());
			} catch(ArithmeticException err) {
				throw new LocalizedIllegalArgumentException(err, PACKAGE_RESOURCES, "TransactionRequest.setAmount.amount.cannotNormalize");
			}
		}
	}
	...
	public void setTaxAmount(BigDecimal taxAmount) {
		if(taxAmount==null) {
			...
			if(taxAmount.compareTo(BigDecimal.ZERO)<0) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "TransactionRequest.setTaxAmount.taxAmount.lessThanZero");
			try {
				this.taxAmount = taxAmount.setScale(currency.getDefaultFractionDigits());
			} catch(ArithmeticException err) {
				throw new LocalizedIllegalArgumentException(err, PACKAGE_RESOURCES, "TransactionRequest.setTaxAmount.taxAmount.cannotNormalize");
			}
		}
	}
	...
	public void setShippingAmount(BigDecimal shippingAmount) {
		if(shippingAmount==null) {
			...
			if(shippingAmount.compareTo(BigDecimal.ZERO)<0) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "TransactionRequest.setShippingAmount.shippingAmount.lessThanZero");
			try {
				this.shippingAmount = shippingAmount.setScale(currency.getDefaultFractionDigits());
			} catch(ArithmeticException err) {
				throw new LocalizedIllegalArgumentException(err, PACKAGE_RESOURCES, "TransactionRequest.setShippingAmount.shippingAmount.cannotNormalize");
			}
		}
	}
	...
	public void setDutyAmount(BigDecimal dutyAmount) {
		if(dutyAmount==null) {
			...
			if(dutyAmount.compareTo(BigDecimal.ZERO)<0) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "TransactionRequest.setDutyAmount.dutyAmount.lessThanZero");
			try {
				this.dutyAmount = dutyAmount.setScale(currency.getDefaultFractionDigits());
			} catch(ArithmeticException err) {
				throw new LocalizedIllegalArgumentException(err, PACKAGE_RESOURCES, "TransactionRequest.setDutyAmount.dutyAmount.cannotNormalize");
			}
		}
	}
	...
	public void setMerchantEmail(String merchantEmail) {
		if(merchantEmail==null) {
			...
			if(!GenericValidator.isEmail(merchantEmail)) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "TransactionRequest.setMerchantEmail.merchantEmail.invalid");
