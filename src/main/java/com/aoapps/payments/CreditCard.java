...
public class CreditCard implements Cloneable {
	...
	public static String validateCardCode(String cardCode) throws LocalizedIllegalArgumentException {
		...
		if(cardCode.length()!=3 && cardCode.length()!=4) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "CreditCard.validateCardCode.cardCode.mustBe3Or4Digits");
		// Each digit must be a number
		for(int c=0;c<cardCode.length();c++) {
			char ch = cardCode.charAt(c);
			if(ch<'0' || ch>'9') throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "CreditCard.validateCardCode.cardCode.mustBeDigits");
		}
		return cardCode;
	}
	...
	public void setEmail(String email) {
		if(email==null || email.length()==0) {
			...
			email = email.trim();
			if(!GenericValidator.isEmail(email)) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "CreditCard.setEmail.email.invalid");
			this.email = email;
		}
	}
	...
	public void setCountryCode(String countryCode) {
		if(countryCode==null || countryCode.length()==0) {
			...
			if(countryCode.length()!=2) throw new LocalizedIllegalArgumentException(PACKAGE_RESOURCES, "CreditCard.setCountryCode.countryCode.mustBe2Digits");
