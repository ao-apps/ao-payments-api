...
public class PropertiesPersistenceMechanism implements PersistenceMechanism {
  ...
  private synchronized void save() throws SQLException {
    try {
      final File newFile = new File(propertiesFilePath + ".new");
      final File file = new File(propertiesFilePath);
      final File backupFile = new File(propertiesFilePath + ".backup");
      ...
      if (file.exists()) {
        FileUtils.rename(file, backupFile);
      }
   ...
   * {@inheritDoc}
   *
   * <p>Card numbers are not persisted to the properties files - encrypted local storage not supported.</p>
   */
  @Override
  public synchronized void updateCreditCard(Principal principal, CreditCard creditCard) throws SQLException {
    ...
    if (internalCreditCard == null) {
      throw new LocalizedSQLException("23000", PACKAGE_RESOURCES, "PersistenceMechanism.updateCardNumber.unableToFindCard", creditCard.getPersistenceUniqueId());
    }
